...
/**
 * Generate and download a PDF expense slip
 */
export const generateExpenseSlipPDF = (expense, mosqueName) => {
    const doc = new jsPDF();
    ...
    doc.setFontSize(12);
    doc.setTextColor(100);
    doc.text("Expenses Receipt", 105, 28, { align: "center" });
    ...
    doc.text(`Voucher ID: #${expense.id ? expense.id.slice(-8).toUpperCase() : 'N/A'}`, 190, 42, { align: "right" });
    ...
    addRow("Purpose:", expense.purpose);
    addRow("Category:", expense.category || 'Other');
    addRow("Paid To:", expense.paidTo);
    addRow("Payment Method:", expense.paymentMethod);
    
    // Amount Highlighted
    yPos += 5;
    doc.setFillColor(240, 253, 244); // Light green bg
    doc.roundedRect(25, yPos - 8, 160, 16, 2, 2, 'F');
    doc.setFont("helvetica", "bold");
    doc.setTextColor(22, 163, 74);
    doc.text("Amount Paid:", 30, yPos);
    doc.text(`Rs ${parseFloat(expense.amount).toLocaleString()}`, 90, yPos);
    ...
    if (expense.notes) {
        ...
        const notesLines = doc.splitTextToSize(expense.notes, 100);
        doc.text(notesLines, 90, yPos);
    }
    ...
    doc.text("Receiver Signature", 138, sigY + 5);

    // Footer
    const finalY = 280;
    doc.setFontSize(8);
    doc.setTextColor(150);
    doc.text("Generated by Masjid Manager System", 105, finalY, { align: "center" });

    // Save
    doc.save(`Expense_Receipt_${expense.purpose.replace(/\s+/g, '_')}_${formattedDate}.pdf`);
};
