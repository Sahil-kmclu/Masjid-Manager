...
function ImamSalary({
  ...
  const stats = useMemo(() => {
    ...
    // Get unique contributing members (by name since we store name, not ID)
    const contributingMembers = new Set(validPayments.map((p) => p.memberName));

    // Calculate top contributors
    const contributorStats = {};
    validPayments.forEach((payment) => {
      const key = payment.memberName || "Unknown";
      ...
      contributorStats[key].totalAmount += parseFloat(payment.amount);
      contributorStats[key].paymentCount += 1;
    });
  ...
  const filteredPayments = useMemo(() => {
    ...
    // Apply search filter (use memberName and memberPhone from payment record)
    if (searchTerm) {
    ...
    // Sort and return (memberName and memberPhone already in payment record from backend)
    return filtered.sort((a, b) => new Date(b.month) - new Date(a.month));
  }, [imamSalaryPayments, searchTerm, dateFilter]);
  ...
  const shareViaWhatsApp = (payment) => {
    ...
    const message = encodeURIComponent(
      `*${mosqueName}*\n\n` +
        ...
        `${t("Amount")}: Rs. ${parseFloat(
          payment.amount
        ).toLocaleString()}\n\n` +
        `${t("Thank you for your contribution!")}`
    );

    const phone = payment.memberPhone;
    const url = phone
      ? `https://wa.me/${
  ...
  const handleDelete = async (payment) => {
    ...
    const phoneToVerify =
      payment.memberPhone && payment.memberPhone !== "-"
        ? payment.memberPhone
        : currentUser.phone;

    const isVerified = await verifyOTP(
      phoneToVerify,
      "delete this salary payment"
    );
    if (isVerified) {
  ...
  return (
    <div className="imam-salary fade-in">
      <div className="page-header">
        <div>
          ...
          <p className="text-muted">
            {t("Track salary contributions from September 2020 onwards")}
          </p>
        </div>
      </div>
      ...
      >
        <div className="stat-card card">
          ...
          <div className="stat-content">
            ...
            <div
              ...
              title={`₹${stats.totalCollected.toLocaleString()}`}
            >
        ...
        <div className="stat-card card">
          ...
          <div className="stat-content">
            ...
            <div className="stat-value">
              {stats.contributingMembers}/{stats.totalMembers}
            </div>
          </div>
        </div>
        ...
        <div className="card" style={{ marginBottom: "var(--spacing-lg)" }}>
          ...
          <div className="top-contributors">
            {stats.topContributors.map((contributor, index) => (
              <div key={contributor.memberName} className="contributor-item">
      ...
      <div className="card">
        <h3>
          {t("Salary Payments")} ({filteredPayments.length})
        </h3>
              ...
              {filteredPayments.length === 0 ? (
                ...
                filteredPayments.map((payment) => (
                  <tr key={payment.id}>
                    ...
                    <td>
                      {new Date(payment.month + "-01").toLocaleDateString(
                        "en-US",
                        {
                          month: "long",
                          year: "numeric",
