...
function MosqueIncome({ mosqueIncome, onDeleteIncome, isReadOnly }) {
  ...
  const stats = useMemo(() => {
    ...
    mosqueIncome.forEach((income) => {
      const category = income.incomeType || "Other";
    ...
    const categoriesBreakdown = Object.values(categoryStats).sort(
  ...
  const filteredIncome = useMemo(() => {
    ...
    if (categoryFilter !== "all") {
      filtered = filtered.filter(
        (income) => income.incomeType === categoryFilter
      );
    }
  ...
  const categories = useMemo(() => {
    const cats = new Set(
      mosqueIncome.map((income) => income.incomeType || "Other")
    );
  ...
  const generateReceipt = (income) => {
    ...
    addLine("Mobile Number", income.donorPhone);
    addLine("Category", income.incomeType);
    addLine("Source", income.source);
    addLine("Amount", `Rs. ${parseFloat(income.amount).toLocaleString()}`);
    if (income.notes) {
      addLine("Notes", income.notes);
    }
  ...
  const shareViaWhatsApp = (income) => {
    ...
    const message = encodeURIComponent(
      `*${mosqueName}*\n\n` +
        ...
        `${t("Category")}: ${t(income.incomeType)}\n\n` +
        `${t("Thank you for your contribution!")}`
    );

    const url = income.donorPhone
      ? `https://wa.me/${
          income.donorPhone.length === 10
            ? "91" + income.donorPhone
            : income.donorPhone
        }?text=${message}`
  ...
  const handleDelete = async (income) => {
    ...
    const phoneToVerify = income.donorPhone || currentUser.phone;
  ...
  return (
    <div className="mosque-income fade-in">
      <div className="page-header">
        <div>
          ...
          <p className="text-muted">
            {t("Track all other income sources for the mosque")}
          </p>
        </div>
      </div>
      ...
      <div className="card">
        <h3>
          {t("Income Records")} ({filteredIncome.length})
        </h3>
                ...
                filteredIncome.map((income) => (
                  <tr key={income.id}>
                    ...
                    <td>
                      ...
                      {income.notes && (
                        <div className="text-muted text-sm">{income.notes}</div>
                      )}
                    </td>
                    <td>
                      <span className="badge badge-primary">
                        {t(income.incomeType || "Other")}
                      </span>
                    </td>
                    <td>
